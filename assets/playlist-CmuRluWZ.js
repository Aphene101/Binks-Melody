import{a as u,b as y,d as p,f as v,c as h,g as f}from"./firebase-DuiegUGH.js";import"./jamendo-HHGNiR1B.js";import"./main-8fpznSiV.js";const w=new URLSearchParams(window.location.search);let c=w.get("id");console.log("Playlist ID from URL:",c);c||(alert("No playlist selected. Redirecting to playlists page."),window.location.href="./playlists.html");const b=document.getElementById("pl-back"),E=document.getElementById("pl-name"),l=document.getElementById("pl-icon"),m=document.getElementById("pl-songs");b.addEventListener("click",()=>{window.location.href="playlists.html"});u.onAuthStateChanged(async t=>{if(!c){console.warn("No playlist id found");return}t&&(t.uid,await L(t.uid,c))});async function L(t,n){console.log("Loading playlists");const r=y(p,"users",t,"playlists",n),a=await v(r);if(!a.exists())return;console.log(a+"The snap is above");const s=a.data();E.textContent=s.name||"Untitled Playlist";let e=[];if(Array.isArray(s.songs)&&s.songs.length)e=s.songs;else{const d=h(p,"users",t,"playlists",n,"songs");e=(await f(d)).docs.map(o=>({id:o.id,...o.data()}))}const i=e[0]?.album_image||e[0]?.albumArt||e[0]?.cover||"";i?(l.style.backgroundImage=`url(${i})`,l.style.backgroundColor="transparent",l.style.backgroundSize="cover",l.style.backgroundPosition="center"):l.style.backgroundImage="none",console.log(e),C(e)}function C(t){m.innerHTML="",t.forEach((n,r)=>{const a=document.createElement("div"),s=document.createElement("div");s.className="song-item";const e=n.name||n.title||"Unknown Title",i=n.artist_name||n.artist||"Unknown Artist",d=n.album_image||n.albumArt||n.cover||"";s.innerHTML=`
      <div class="song-left">
